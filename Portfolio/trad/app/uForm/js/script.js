...
function validEmail(item, empty = true) {
  ...
  if (!item.value.match(emailRegExp)) return errMsg;
  return true;
}
  ...
  for (let i = 0; i < files; i++) {
    if ((file = files[i])) {
      if (file.size > limitByte) {
        return (
          "превышен допустимый лимит размера файла: " + sizeLimit + " " + unit
        );
      }
    }
  }
...
function validSizeAllFiles(item, sizeLimit, unit = "КБ") {
  ...
  if (files) {
    ...
    if (fullSize > limitByte) {
      errMsg =
        "превышен допустимый лимит общего размера файлов: " +
        sizeLimit +
        " " +
        unit;
    }
...
function printMessageText(instance, status) {
  ...
  uForm.html("");
  uForm.append(
    '<p id="uForm__message-text' +
      instance.uFormPrefix +
      ...
      $(function() {
        for (let uFormId in uForms) {
          ...
          formInstance.submit(function(event) {
            ...
            if (!valid) {
              // printError(this.uFormPrefix, "Некоторые поля заполненные не корректно");
